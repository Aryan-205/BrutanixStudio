...
import { projectCaseStudies, projectsHero } from "@/data/projectsPageContent";
...
export default function ProjectsHero() {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      setActiveIndex((prev) => (prev + 1) % projectCaseStudies.length);
    }, CYCLE_MS);
    return () => clearInterval(timer);
  }, [reduce]);

  const active = projectCaseStudies[activeIndex];
    ...
    <section className="relative flex min-h-[85vh] flex-col items-center justify-center bg-white px-6 pb-20 pt-36 text-center md:min-h-[90vh] md:pb-28 md:pt-44">
      ...
      >
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <p className="mx-auto mt-5 max-w-md text-sm leading-relaxed text-neutral-500 md:text-base">
                {active.headline}{" "}
                <span className="font-semibold text-[#5210F8]">{active.highlight}</span>.
              </p>
            </motion.div>
          </AnimatePresence>
        </div>
        ...
        <div className="mt-8 flex items-center justify-center gap-1.5">
          {projectCaseStudies.map((_, i) => (
            <button
              key={i}
              type="button"
              onClick={() => setActiveIndex(i)}
              aria-label={`View ${projectCaseStudies[i].name}`}
