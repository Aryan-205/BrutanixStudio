    ...
    <section className="bg-[#F9F9F9] text-black px-6 md:px-12 py-20">
      <div className="max-w-7xl mx-auto flex flex-col gap-12 md:gap-20">
        {projects.map((project, index) => (
          <Reveal
            key={index}
            x={project.reverse ? 36 : -36}
            className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center"
          >
            <div
              className={`flex flex-col gap-4 ${project.reverse ? "md:order-2 md:pl-20" : "md:pr-20"}`}
            >
            ...
            <div
              className={`relative aspect-video w-full overflow-hidden grayscale hover:grayscale-0 transition-all duration-700 ease-in-out ${project.reverse ? "md:order-1" : ""}`}
            >
